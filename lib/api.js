...
function substituteResponse(string, json) {
    for (var key in api.vars) {
        if (!api.vars.hasOwnProperty(key)) continue
        if (json)
            string = string.replace('[' + key + ']', JSON.stringify(api.vars[key]))
...
function headerCapture(string) {
    var match
    if (match = /^\[([A-Z_]+)=\]$/.exec(string)) {
        return { regex: /(.*)/, captures: [match[1]] }
    }
}
...
api.matchers = function (batch, params) {
    ...
    if (params.body) {
        batch[params.body.replace(/\s+/g, " ")] = function(error, result) {
            ...
            pattern = pattern.replace(/\[([A-Z_]+)=\]/, '{ "$CAPTURE": "$1" }')
...
api.makeBatch = function (test) {
    ...
    batch[test.label] = topic = { topic: api.call(test.req) }
    if (test.resp)
...
api.vars.USERID = "4e36a8ad6f6b71402900000f"
